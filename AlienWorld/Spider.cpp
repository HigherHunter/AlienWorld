		...
		scale)
{
	...
	m_pStateMachine = new StateMachine<Spider>(this);
	m_pStateMachine->SetCurrentState(SpiderHide::Instance());
}
...
void Spider::GotHit(float damage)
{
	// Play sound
...
void Spider::MarkDead()
{
	...
	// Play sound
	SoundManager::instance().playSpiderDead();
}

bool Spider::ReadyToDelete()
{
	if (!alive)
	{
...
void Spider::MakeAttackSound()
{
	SoundManager::instance().playSpiderAttack();
}

void Spider::LoadAnimations()
{
	for (int i = 0; i < 64; i++)
	{
		...
		str_number << i + 1;
		std::string fileName = "Assets\\Sprites\\Spider\\Running\\spider_walk_" + str_number.str() + ".png";
		runningAnim.push_back(LoadTexture((char*)fileName.c_str()));

		if (i < 63)
		{
			// Get images for dead animation
			fileName = "Assets\\Sprites\\Spider\\Dead\\spider_dead_" + str_number.str() + ".png";
